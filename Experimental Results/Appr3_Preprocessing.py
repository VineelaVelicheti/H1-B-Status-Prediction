...
data = pd.read_csv("C:/Users/yashas/DMLatest/Final/H-1B_data.csv")
...
#Getting info to check if dataset is imbalanced or not 
...
df_majority = data.loc[data['CASE_STATUS'] == 'CERTIFIED'] 
df_minority = data.loc[data['CASE_STATUS'] == 'DENIED'] 
...
data = pd.concat([df_minority, df_majority_downsampled],ignore_index=True)
...
c2.fit(train_data,Y_train)
GradB_Y =c2.predict(test_data)
...
print('Time in seconds:',time.clock()-start)
...
Mlp_model = mlp.fit(train_data,Y_train)
pred_mlp = Mlp_model.predict(test_data)
...
SGD_model = SGDClassifier().fit(train_data,Y_train)
pred_sgd = SGD_model.predict(test_data)
